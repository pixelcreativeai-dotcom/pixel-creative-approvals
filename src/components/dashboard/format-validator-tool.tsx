...
import { CheckCircle2, AlertCircle, UploadCloud, X, Loader2 } from 'lucide-react';
...
const placeholderImage = PlaceHolderImages.find(p => p.id === 'asset-upload-preview');

export function FormatValidatorTool() {
  ...
  const platforms = [...new Set(formatSpecs.map(s => s.platform))];
          ...
          {preview ? (
            <div className="relative group">
                {file?.type.startsWith('video/') ? (
                    <video src={preview} controls className="w-full rounded-lg aspect-video object-contain bg-gray-100" />
                ) : (
                    <Image
                        src={preview}
                        alt="Preview"
